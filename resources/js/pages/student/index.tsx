...
export default function StudentIndex() {
    ...
    const handleSubmit = (e: React.FormEvent)=>{
        e.preventDefault();
        if (isEdit && form.id) {
            router.put(`/students/update/${form.id}`, form,{
                onSuccess: handleClose,
            });

        }else{
            router.post(`/students/create`, form, {
                onSuccess:handleClose,
            });
        };
                    ...
                    <DialogContent>
                        ...
                        <form className='space-y-4' onSubmit={handleSubmit}>
                            ...
                            <div>
                                ...
                                <input className='border-input flex h-9 w-full min-w-0 rounded-md border bg-transparent px-3 py-1 text-base shadow-xs transition=[color, box-shadow outline-none md:text-sm' type="text"  id='grade' name='grade' value={form.grade} onChange={handleChange} required/>
                            </div>
